...
async function loadAuditSummary() {
  ...
  let rows;
  try {
    const res = await fetch("./audit.json", { cache: "no-store" });
    if (!res.ok) return;
    rows = await res.json();
  } catch (_) {
    return;
  ...
  container.replaceChildren(
    ...rows.map((r) => {
      const cls = lightClass[String(r.light).toLowerCase()] || "risk-unknown";
      const item = el("div", "briefing-item");
      item.append(
        el("span", "briefing-id", r.category),
        el("span", `status-pill ${cls}`, r.light),
        el("span", "briefing-why", r.summary)
      );
      return item;
    })
  );
