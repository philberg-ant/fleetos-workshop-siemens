...
async function loadIncidentCount() {
  ...
  try {
    ...
    el.textContent = incidents.filter((i) => !i.resolved).length;
  } catch (_) {
    // API not running — leave the placeholder
  }
}

async function loadOpsFeed() {
  const container = document.getElementById("ops-feed");
  let status;
  try {
    const res = await fetch("./live/ops_status.json", { cache: "no-store" });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    status = await res.json();
  } catch (_) {
    // No loop has written the file yet — keep the waiting message
    return;
  }
  ...
  const row = el("div", "ops-feed-row");
  row.append(
    el("span", "ops-feed-count", `${status.open_count ?? "?"} open`),
    el("span", "ops-feed-last", status.last_incident ?? "—"),
    el("span", "ops-feed-updated", `updated at ${status.updated_at ?? "?"}`)
  );
...
loadIncidentCount();
loadOpsFeed();
setInterval(loadOpsFeed, 15000);
setInterval(loadIncidentCount, 15000);
